...
function Navbar() {
  ...
  const darkModeSwitch = (e) => {
    if (e.target.checked) {
      ...
      html[0].setAttribute('data-theme', 'lofi')
    }
  }

  return (
    <header className={`bg-base-100 border-b-2 z-10 ${location === "/login" || location === "/register" ? "hidden":"block"}`}>
      <div className="container navbar">
        <div className="navbar-start">
          <Link to="/" className="btn btn-ghost pl-1 hover:bg-white normal-case text-xl font-semibold italic">
            BlitzBiz
          </Link>

          <label className="swap swap-rotate">
            <input type="checkbox" onClick={darkModeSwitch}/>
        ...
        <div className="navbar-center hidden lg:block">
          <ul className="menu menu-horizontal px-1">
            <li>
              <Link className={`${location === "/" ? "font-semibold":""}`} to="/">Beranda</Link>
            </li>
            <li>
              <Link className={`${location === "/eksplorasi" ? "font-semibold":""}`} to="/eksplorasi">Eksplorasi</Link>
            </li>
            <li>
              <Link className={`${location === "/acara" ? "font-semibold":""}`} to="/acara">Acara & Program</Link>
            </li>
          </ul>
        </div>
        ...
        <div className="navbar-end">
          <div className="dropdown dropdown-end">
            ...
            >
              <li>
                <Link className="justify-between" to={"/login"}>Akun saya</Link>
              </li>
              <li>
                <a>Listing saya</a>
              </li>
        ...
        <div className="btm-nav z-10 md:flex lg:hidden">
          ...
          <Link className={`${location === "/eksplorasi" ? "active":""}`} to="/eksplorasi">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
          ...
          <Link className={`${location === "/acara" ? "active":""}`} to="/acara">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
